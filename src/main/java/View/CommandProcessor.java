            ...
            if (ConsoleCommands.EXIT.getStringMatcher(command).matches()) {
                ...
                if (matcher.find()) {
                    ...
                    sellerController.showPersonalInfo();
                     if (ConsoleCommands.EDIT_SELLER_PERSONAL_INFO.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.EDIT_SELLER_PERSONAL_INFO.getStringMatcher(command);
                        if (matcher.find()) {
                            String field = matcher.group(1);
                ...
                if (matcher.find()) {
                    //TODO
                     if (ConsoleCommands.VIEW_SELLER_PRODUCTS.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.VIEW_SELLER_PRODUCTS.getStringMatcher(command);
                        if (matcher.find()) {
                        ...
                        Matcher subMatcher = ConsoleCommands.VIEW_BUYERS_SELLER_PRODUCTS.getStringMatcher(command);
                        if (matcher.find()) {
                            String buyer = matcher.group(1);
                            //TODO
                        }
                    } else if (ConsoleCommands.EDIT_SELLER_PRODUCTS.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.EDIT_SELLER_PRODUCTS.getStringMatcher(command);
                        if (matcher.find()) {
                            String productId = matcher.group(1);
                            controller.SellerController sellerController = null;
                ...
                if (matcher.find()) {
                    ...
                    seller.getAllOffers();
                     if (ConsoleCommands.VIEW_SELLER_OFFS.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.VIEW_SELLER_OFFS.getStringMatcher(command);
                        if (matcher.find()) {
                            //TODO
                        }
                    } else if (ConsoleCommands.EDIT_SELLER_OFFS.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.EDIT_SELLER_OFFS.getStringMatcher(command);
                        if (matcher.find()) {
                        ...
                        Matcher subMatcher = ConsoleCommands.ADD_SELLER_OFFS.getStringMatcher(command);
                        if (matcher.find()) {
                ...
                if (matcher.find()) {
                    controller.ShopperController shopperController = null;
                    shopperController.showPersonalInfo();
                     if (ConsoleCommands.EDIT_SHOPPER_PERSONAL_INFO.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.EDIT_SHOPPER_PERSONAL_INFO.getStringMatcher(command);
                        if (matcher.find()) {
                            String field = matcher.group(1);
                            shopperController.changePersonalInfo(field);
                        }
                    }
                }
                ...
                if (matcher.find()) {
                    controller.ShopperController shopperController = null;
                    shopperController.showCredit();
                     if (ConsoleCommands.SHOW_SHOPPER_PRODUCTS.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.SHOW_SHOPPER_PRODUCTS.getStringMatcher(command);
                        if (matcher.find()) {
                            //TODO
                        }
                    } else if (ConsoleCommands.VIEW_SHOPPER_PRODUCTS.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.VIEW_SHOPPER_PRODUCTS.getStringMatcher(command);
                        if (matcher.find()) {
                            //TODO
                        }
                    } else if (ConsoleCommands.INCREASE_SHOPPER_PRODUCTS.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.INCREASE_SHOPPER_PRODUCTS.getStringMatcher(command);
                        if (matcher.find()) {
                            //TODO
                        }
                    } else if (ConsoleCommands.DECREASE_SHOPPER_PRODUCTS.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.DECREASE_SHOPPER_PRODUCTS.getStringMatcher(command);
                        if (matcher.find()) {
                            //TODO
                        }
                    } else if (ConsoleCommands.SHOW_SHOPPER_TOTAL_PRICE.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.SHOW_SHOPPER_TOTAL_PRICE.getStringMatcher(command);
                        if (matcher.find()) {
                            //TODO
                        }
                    } else if (ConsoleCommands.PURCHASE_SHOPPER.getStringMatcher(command).matches()) {
                        Matcher subMatcher = ConsoleCommands.PURCHASE_SHOPPER.getStringMatcher(command);
                        if (matcher.find()) {
                            controller.ShopperController shopperController1 = null;
                            shopperController.purchase();
                        }
                    }
                }
